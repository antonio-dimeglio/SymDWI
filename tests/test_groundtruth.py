...
def test_single_bundle_has_one_peak_aligned_with_axis():
    ...
    geom = symdwi.BundleGeometry(pts, n_streamlines=200, radius=4.0, dispersion=0.2, seed=0)
...
def test_crossing_produces_two_peaks_somewhere():
    ...
    cc = symdwi.Bundle(symdwi.BundleGeometry(
        np.array([[4, 15, 18], [15, 15, 18], [26, 15, 18]], float),
        n_streamlines=250, radius=4.0, dispersion=0.3, seed=0))
    cst = symdwi.Bundle(symdwi.BundleGeometry(
        np.array([[15, 4, 18], [15, 15, 18], [15, 26, 18]], float),
        n_streamlines=250, radius=4.0, dispersion=0.3, seed=1))
    bvals, bvecs = symdwi.generate_bvals_bvecs(shells=[(2000, 40)], n_b0=2, seed=1)
    scan = symdwi.ScanParameters(te_ms=80.0)
    tissue = symdwi.TissueParameters(axon_radius=1.0)
    _, _, gt = symdwi.simulate_dwi(
        [cc, cst], bvals, bvecs, scan, tissue=tissue, origin=np.zeros(3), dims=(30, 30, 36),
        voxel_size=1.0, snr=None, sphere="repulsion724", return_groundtruth=True,
    )
    m = gt["wm_mask"] > 0
